...
- **View Plugin**: Source-based routing using CoreDNS view plugin for network isolation
  ...
  - Queries from pod network see the internal proxy ClusterIP when configured;
    otherwise HCP records are hidden and queries go upstream
- **Hosts Plugin**: Maps HCP control-plane FQDNs and conditional Kubernetes
  aliases to the Envoy L4 proxy IP in the VLAN view
...
2. **Default View** (`view default`):
   ...
   - HCP endpoints point to the internal proxy ClusterIP when
     `internalProxyService` is configured
   - If no internal proxy is configured, no static HCP records are emitted in
     this view and queries are forwarded upstream
...
.:53 {
    ...
    view multus {
        ...
        # HCP domain with static entries generated by Infra
    ...
    # Default view for pod network. When configured, generated HCP records use
    # the internal proxy Service ClusterIP instead of the VLAN address.
    view default {
        . {
            forward . 8.8.8.8
            cache 30s
...
In production, the Infra controller creates a ConfigMap containing the Corefile
from `Infra` and its `InfraClusterAttachment` resources. For an Infra named
`my-cluster`, the generated ConfigMap is `my-cluster-dns-dns-config` in the
Infra namespace and the generated Deployment and Service are both
`my-cluster-dns`:
...
kind: ConfigMap
metadata:
  name: my-cluster-dns-dns-config
  namespace: clusters
data:
  Corefile: |
    .:53 {
...
- The controller generates the Corefile and static records; this package does
