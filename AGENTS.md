...
- `make run` — run manager locally against current cluster (`go run ./main.go manager`); requires `make install` first. Debug: `make run -- --zap-log-level=debug`
- `make build` — builds `bin/oooi`
- `make container-build` — ko multi-arch `linux/amd64,linux/arm64` (`KO_DOCKER_REPO=quay.io/cldmnky/oooi`, base `registry.access.redhat.com/ubi9/ubi:9.4` in `.ko.yaml`). E2E variant: `make container-build-e2e`
...
- `make setup-test-e2e` — creates/reuses Kind cluster `$KIND_CLUSTER` (default `oooi-test-e2e`; uses `hack/kind-config.yaml` or `hack/kind-config-podman.yaml` if `PODMAN_RUNTIME=true`), installs CNI plugins (`test/e2e/install-cni-plugins.sh`), Calico `v3.29.1`, Multus `v4.2.3`, creates NADs (`test/e2e/test-nads.yaml`). Idempotent.
- `make test-e2e` — runs `setup-test-e2e` + `manifests generate fmt vet` + `KIND_CLUSTER=$KIND_CLUSTER CERT_MANAGER_INSTALL_SKIP=true go test ./test/e2e/ -v -ginkgo.v` then auto `make cleanup-test-e2e`. Env: `KIND_CLUSTER`, `CALICO_VERSION`, `MULTUS_VERSION`, `PODMAN_RUNTIME`, `CERT_MANAGER_INSTALL_SKIP`.
...
- OwnerReferences required on all created resources (`ctrl.SetControllerReference`) for cascade delete on Infra removal.
